...
class Raintpl_Class extends Core{
  ...
  static $cache_it = false;
    ...
    if($this->config('rewrite_template_path')){
			$path = $this->config('rewrite_template_path');
		}
		else{
		...
		self::configure("tpl_dir", $path );
		self::configure("cache_dir", $this->config('user_path').'cache/templates/');
		self::configure("tpl_ext", 'php');
		self::configure("path_replace", false);
		self::configure("cache_it", false);
	...
	if($this->config('rewrite_template_path')){
        ...
        // rain get the output buffer to save the output in the cache or to return it as string
            ...
            if( $this->cache && $cache_it)
                file_put_contents( $this->tpl['cache_filename'], "<?php if(!class_exists('raintpl_Class')){exit;} ?>" . $raintpl_contents );
	...
	*/
	...
	protected function compileFile( $tpl_basename, $tpl_basedir, $tpl_filename, $cache_dir, $compiled_filename ){
		...
		//compile template
		$template_compiled = "<?php if(!class_exists('raintpl_Class')){exit;}?>" . $this->compileTemplate( $template_code, $tpl_basedir );
	...
	protected function compileCode( $parsed_code ){
        ...
        $loop_level = 0;
	 	...
	 	while( $html = array_shift( $parsed_code ) ){
			...
			//lang
			elseif( preg_match( '/\{LANG="([^(]*)(\([^)]*\)){0,1}"\}/', $html, $code ) ){
				...
				$langCode = $code[ 1 ];
                                ...
                                if(stripos($langCode,".")){
                                    ...
                                    $compiled_code .= $this->lang($langCode[0],$langCode[1]);
                                    
                                }
                                else{
                                    
                                    $compiled_code .= $this->lang($langCode[0]);
