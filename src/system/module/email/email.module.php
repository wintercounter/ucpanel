 ...
 * @package PHPMailer
 */
class Email_Module extends Core{
 ...
 /**
  ...
  protected function AddAnAddress($kind, $address, $name = '') {
    if (!preg_match('/^(to|cc|bcc|Reply-To)$/', $kind)) {
      $this->SetError($this->Language('Invalid recipient array').': '.$kind);
      if ($this->exceptions) {
        throw new phpmailerException('Invalid recipient array: ' . $kind);
      }
      if ($this->SMTPDebug) {
        $this->edebug($this->Language('Invalid recipient array').': '.$kind);
      }
      return false;
    }
    ...
    if (!$this->ValidateAddress($address)) {
      $this->SetError($this->Language('invalid_address').': '. $address);
      if ($this->exceptions) {
        throw new phpmailerException($this->Language('invalid_address').': '.$address);
      }
      if ($this->SMTPDebug) {
        $this->edebug($this->Language('invalid_address').': '.$address);
      }
      return false;
    }
    if ($kind != 'Reply-To') {
 ...
 */
  ...
  public function PreSend() {
    try {
      ...
      if ((count($this->to) + count($this->cc) + count($this->bcc)) < 1) {
        throw new phpmailerException($this->Language('provide_address'), self::STOP_CRITICAL);
      }
      ...
      if (empty($this->Body)) {
        throw new phpmailerException($this->Language('empty_message'), self::STOP_CRITICAL);
      }
    ...
    if ($this->SingleTo === true) {
      foreach ($this->SingleToArray as $val) {
        if(!@$mail = popen($sendmail, 'w')) {
          throw new phpmailerException($this->Language('execute') . $this->Sendmail, self::STOP_CRITICAL);
        }
        fputs($mail, "To: " . $val . "\n");
        fputs($mail, $header);
        fputs($mail, $body);
  ...
  protected function MailSend($header, $body) {
    ...
    if(!$rt) {
      throw new phpmailerException($this->Language('instantiate'), self::STOP_CRITICAL);
    }
    return true;
  }
  ...
  protected function SmtpSend($header, $body) {
    ...
    if(!$this->SmtpConnect()) {
      throw new phpmailerException($this->Language('smtp_connect_failed'), self::STOP_CRITICAL);
    }
    $smtp_from = ($this->Sender == '') ? $this->From : $this->Sender;
    if(!$this->smtp->Mail($smtp_from)) {
      throw new phpmailerException($this->Language('from_failed') . $smtp_from, self::STOP_CRITICAL);
    }
    ...
    if (count($bad_rcpt) > 0 ) { //Create error message for any bad addresses
      ...
      throw new phpmailerException($this->Language('recipients_failed') . $badaddresses);
    }
    if(!$this->smtp->Data($header . $body)) {
      throw new phpmailerException($this->Language('data_not_accepted'), self::STOP_CRITICAL);
    }
        ...
        if ($this->smtp->Connect(($ssl ? 'ssl://':'').$host, $port, $this->Timeout)) {
          ...
          if ($tls) {
            if (!$this->smtp->StartTLS()) {
              throw new phpmailerException($this->Language('tls'));
            }
          ...
          if ($this->SMTPAuth) {
            if (!$this->smtp->Authenticate($this->Username, $this->Password, $this->AuthType,
              ...
              throw new phpmailerException($this->Language('authenticate'));
            }
          }
        }
        $index++;
      if (!$connection) {
        throw new phpmailerException($this->Language('connect_host'));
      }
      }
    } catch (phpmailerException $e) {
      $this->smtp->Reset();
  ...
  */
    ...
    if ($this->IsError()) {
      ...
      try {
        ...
        if (@openssl_pkcs7_sign($file, $signed, "file://".$this->sign_cert_file, array("file://".$this->sign_key_file, $this->sign_key_pass), NULL)) {
          ...
          @unlink($file);
          @unlink($signed);
          throw new phpmailerException($this->Language("signing").openssl_error_string());
        }
      } catch (phpmailerException $e) {
        $body = '';
        if ($this->exceptions) {
  ...
  public function AddAttachment($path, $name = '', $encoding = 'base64', $type = 'application/octet-stream') {
    try {
      if ( !@is_file($path) ) {
        throw new phpmailerException($this->Language('file_access') . $path, self::STOP_CONTINUE);
      }
  ...
  */
  ...
  protected function EncodeFile($path, $encoding = 'base64') {
    try {
      if (!is_readable($path)) {
        throw new phpmailerException($this->Language('file_open') . $path, self::STOP_CONTINUE);
      }
  ...
  public function EncodeString($str, $encoding = 'base64') {
    ...
    switch(strtolower($encoding)) {
      ...
      case 'quoted-printable':
        ...
        break;
      default:
        $this->SetError($this->Language('encoding') . $encoding);
        break;
    }
    return $encoded;
  }
  ...
  */
  ...
  public function AddEmbeddedImage($path, $cid, $name = '', $encoding = 'base64', $type = 'application/octet-stream') {
    ...
    if ( !@is_file($path) ) {
      $this->SetError($this->Language('file_access') . $path);
      return false;
    }
  ...
  protected function SetError($msg) {
    ...
    if ($this->Mailer == 'smtp' and !is_null($this->smtp)) {
      ...
      if (!empty($lasterror) and array_key_exists('smtp_msg', $lasterror)) {
        $msg .= '<p>' . $this->Language('smtp_error') . $lasterror['smtp_msg'] . "</p>\n";
      }
    }
    $this->ErrorInfo = $msg;
  }
   ...
   * @return string
   */
  protected function Language($key) {
  ...
  public function set($name, $value = '') {
    try {
      if (isset($this->$name) ) {
        ...
        throw new phpmailerException($this->Language('variable_set') . $name, self::STOP_CRITICAL);
      }
    } catch (Exception $e) {
      $this->SetError($e->getMessage());
      if ($e->getCode() == self::STOP_CRITICAL) {
