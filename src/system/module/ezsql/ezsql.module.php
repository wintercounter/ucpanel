	...
	*/
	...
	class Ezsqlcore extends Core {
		...
		*/
		...
		function get_var($query=null,$x=0,$y=0)
		{
			...
			if($this->config('debug')){
				$this->plugin('profiler')->profiler->queryCount += 1;
				$start = $this->plugin('profiler')->profiler->getTime();
				$q = $query.'|||[Type: Get Var]';
				$this->plugin('profiler')->profiler->logQuery($q, $start);
			}
		...
		*/
		...
		function get_insert_ready($table,$data=array()){
		...
		function get_update_ready($table,$data=array()){
			$where = '';
			if(isset($data['WHERE'])){
